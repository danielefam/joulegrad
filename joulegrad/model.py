...
class ModelEnergyRegularizer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.register_buffer(
            "_device_anchor", torch.empty(0, device=model_device), persistent=False
        )
    ...
    def _evaluate(self, masks, *, include_details):
        ...
        if self._linear_specs:
            ...
            try:
                energies = self.lookup.linear_batch(
                    _stack_scalars(
                        effective_inputs, device=device, dtype=self.lookup.dtype
                    ),
                    _stack_scalars(
                        effective_outputs, device=device, dtype=self.lookup.dtype
                    ),
                )
            except ValueError:
                ...
                for spec, effective_input, effective_output in zip(
                    ...
                    try:
                        energy = self.lookup.linear(effective_input, effective_output)
                    except ValueError:
                        continue
                ...
                energies = torch.stack(scalar_energies) if scalar_energies else None
        ...
        for (kernel_size, stride, padding), specs in self._conv_specs.items():
            ...
            try:
                energies = self.lookup.conv2d_batch(
                    ...
                    torch.tensor(
                        [spec.input_width for spec in specs], device=device
                    ),
        ...
        total = (
            torch.cat(energy_batches).sum()
            if energy_batches
            else torch.zeros((), device=device, dtype=self.lookup.dtype)
        )
        if include_details:
    ...
    def estimate(self, masks=None):
        """Return total and per-layer estimates for diagnostics."""
...
def estimate_model_energy(
    ...
    *,
    ...
    """Estimate explicitly selected modules without inferring connectivity."""
